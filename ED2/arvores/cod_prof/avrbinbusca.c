...
Arv* arv_remove (Arv* r, int c){
  ...
  if (arv_vazia(r)){
    return NULL;
     ...
     if (c == r->info){ // achou elemento a ser removido
         // remoção em folha
         if (arv_vazia(r->esq) && arv_vazia(r->dir)){
              free(r);
              return NULL;
         }     
         else{  // remoção em nó interno (esquerda vazia)
            if (arv_vazia(r->esq)){
                p = r;
                r = r->dir;
                free(p);
...
int ebinaria (arv *tree)
{
  if(tree != NULL)
  {
    if(tree->esq != NULL)
    {
      if(tree->esq->info < tree->info)
        ebinaria(tree->esq);
      else
        return 1;
    }
